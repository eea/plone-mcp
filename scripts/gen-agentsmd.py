        ...
        if match:
            ...
            if not os.path.isabs(referenced_file_path):
                ...
                if input_dir:
                    ...
                    full_referenced_path = os.path.join(
                        os.getcwd(), referenced_file_path
                    )
            else:
            ...
            if (
                not full_referenced_path.startswith(os.getcwd() + os.sep)
                and not full_referenced_path == os.getcwd()
            ):
                print(
                    f"Warning: Referenced file '{full_referenced_path}' is outside the current project directory. Ignoring its content.",
                    file=os.sys.stderr,
                )
                output_lines.append(line)
                output_lines.append(
                    f"<!-- Warning: Referenced file '{path_for_comment}' is outside the project and was ignored. -->\n"
                )
            ...
            try:
                with open(full_referenced_path, "r", encoding="utf-8") as f_ref:
                    ...
                    output_lines.append(
                        f"<!-- End of import from: {path_for_comment} -->\n"
                    )
            except FileNotFoundError:
                print(
                    f"Warning: Referenced file '{full_referenced_path}' not found. Keeping placeholder.",
                    file=os.sys.stderr,
                )
                output_lines.append(line)
                output_lines.append(
                    f"<!-- Warning: Referenced file '{relative_referenced_path_for_comment}' not found during AGENTS.md generation. -->\n"
                )
            except Exception as e:
                print(
                    f"Warning: Error reading referenced file '{full_referenced_path}': {e}. Keeping placeholder.",
                    file=os.sys.stderr,
                )
                output_lines.append(line)
                output_lines.append(
                    f"<!-- Warning: Error reading referenced file '{relative_referenced_path_for_comment}': {e} during AGENTS.md generation. -->\n"
                )
        else:
            output_lines.append(line)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate AGENTS.md by embedding content from referenced files."
    )
    parser.add_argument(
        "-i",
        "--input",
